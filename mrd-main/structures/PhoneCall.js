...
module.exports = class PhoneCall {
	...
	async start() {
        ...
        .setTitle(`Incoming call from ${this.origin.guild.name}`)
        .setDescription(`Type yes or no to answer or decline this call`)
        .setColor('#7FFFD4')
		await this.origin.send(`**☎️ Calling ${this.recipient.guild.name}!**`);
		await this.recipient.send(answerEmbed);
	...
	send(channel, msg, hasText, hasImage, hasEmbed) {
        //if (message.content.includes('discord.gg/' || 'youtube.com/')) message.lineReply('You can not be promoting your links')
